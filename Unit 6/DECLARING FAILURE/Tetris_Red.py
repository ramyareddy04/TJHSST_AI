...
def heuristic(board, strategy):
    ...
    val = 0
    val += a * sum(tops)/10 # avg col
    val += b * max(tops) # highest col
    val += c * sum(holes) # sum holes 
    val += d * max([abs(tops[i]-tops[i+1]) for i in range(0,9)]) # max well
    val += e * rows.count(10) # rows cleared
    val += f * sum([abs(tops[i]-tops[i+1]) for i in range(0,9)]) # bumpiness
    val += g * sum([1 for i in range(0,9) if abs(tops[i]-tops[i+1])!=0]) # num wells
    return val
...
def generate_population():
    ...
    while len(population)!=POPULATION_SIZE:
        n = [random.uniform(-0.5, 0.5) for i in range(7)]
