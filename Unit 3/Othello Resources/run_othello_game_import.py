...
p1 = "oth_custom_red_import.py"
#p1 = "other_custom_server_import.py"
...
FANCIES = {'x': '\x1b[38;2;0;0;0m●\x1b[0m', 'o': '●'}

def nicely_print2(board, turn):
    board_rows = [board[x:x + 8] for x in range(0, 64, 8)]
    board_moves = possible_moves(board, 'xo'[turn])
    for row_idx, row in enumerate(board_rows):
        for char_idx, char in enumerate(row):
            if char != '.':
                print(FANCIES[char], end=' ')
            elif row_idx * 8 + char_idx in board_moves:
                print('\x1b[38;2;0;128;128m●\x1b[0m', end=' ')
            else:
                print('.', end=' ')
        print()
