...
def get_score(board, token):
    ...
    if board.count('.')==0 or movesX+movesY==0:
        ...
        if board.count('.') < 8:
            ...
            # 11, 12, 30, 10
            diffP = 8*(piecesX-piecesY)/(piecesX+piecesY)
            diffM = 10*(movesX-movesY)/(movesX+movesY) if movesX+movesY!=0 else 0
            corners = [board[i] for i in [0,7,56,63]]
            if ('x' or 'o') in corners:
                diffC = 30*(corners.count('x')-corners.count('o'))/(corners.count('x')+corners.count('o'))
                opp = 0
                my = 0
...
### code for 75% against random assignment
depth = 1

print(int(random.choice(possible_moves(board, player))))
