...
def game_over(string, currentPlayer):
	x = ''
	o = ''
	...
	for state in winStates:
		if state[0] in x and state[1] in x and state[2] in x: 
			return 1 if currentPlayer == 'X' else -1
...
def do_game(board, isCompNext):
	...
	if isCompNext:
		if computer == 'X':	ways = [-negamax(board[:idx] + computer + board[idx+1:], not isCompNext) for idx in pos]
		else: ways = [negamax(board[:idx] + computer + board[idx+1:], not isCompNext) for idx in pos]
		showMoves(ways, pos)
